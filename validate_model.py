...
# Reset index để tránh lỗi iloc out-of-bounds
...
if 'date' in df_raw.columns:
    print(f"  Khoảng ngày: {df_raw['date'].min()} → {df_raw['date'].max()}")
...
if abs(gap_ts) < 0.05:
    print(f"\n  ✅ Time-series acc tương đương random split → 77.13% có cơ sở")
elif gap_ts > 0.10:
    print(f"\n  ⚠️  Random split overestimate {gap_ts*100:.1f}% → con số 77% CÓ THỂ LÀ GIẢ")
else:
    print(f"\n  ⚡ Random split cao hơn time-series {gap_ts*100:.1f}% — chênh lệch nhỏ")
...
if 'date' in df_raw.columns:
    ...
    df_test2['year'] = dates.loc[X_test.index].values
    df_test2['year'] = pd.to_datetime(df_test2['year']).dt.year
...
print(SEP)
print(f"""
  Random split accuracy      : {test_acc*100:.2f}%
